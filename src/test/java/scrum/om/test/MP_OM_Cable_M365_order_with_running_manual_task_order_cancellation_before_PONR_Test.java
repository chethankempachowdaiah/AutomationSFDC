...
public class MP_OM_Cable_M365_order_with_running_manual_task_order_cancellation_before_PONR_Test extends BaseBrowser {
	 ...
	 * 
	 *                              6. Cable + M365 order with running manual task - order cancellation before PONR
     								- Delivery agent is running Review Order Details task
								     - AE cancels the order
								     - Notification goes to order owner (Company Authorized By) and to the agent who runs the task if different from the owner
								     - Original order goes into status Cancel Requested
								     - Delivery agent receives notification, validates the order and completes the task
								     - Review order Details task becomes Cancelled, the rest of the tasks - Discarded
	...
	public void test_Validate_Cable_M365_order_with_running_manual_task_order_cancellation_before_PONR(Hashtable<String, String> dataTable)
		...
		//
		sf.dataInput.orderNumber = "00385548";
		//		sf.omData.canceledOrderNumber = "00069796";
		//		sf.omData.supersededOrderNumber  = "00071256";
		...
		//2. *****verify task is running and cancels the order****************************
		...
		//***RDI cancel Order an verify task status is cancel requested********
	...
	//	sfdc.orderRelated.verifyOrchestrationPlanInOrder();
		...
		//*********Verify all pending and ready task is in frozen state, running task will be in running state, 
		// completed task will be in completed state
	//	sfdc.orchPlan.verifyAllTaskItemsStaus("Frozen");	
		...
		// verify notification SD receives after AE cancels the order
		...
		// Complete the running task, verify the status(order is cancelled complete the task) 
		// after clicking the checkbox check box for verify status button
		sfdc.orchPlan.pickCompleteCircuitDesign("Review Order Details");
